...
class VFCameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    // MARK: -  properties
    ...
    var cameraOutputQuality: AVCaptureSession.Preset = .high {
        didSet {
     ...
     */
    @discardableResult  func addPreviewLayerToView(_ view: UIView) -> CameraState {
        return addPreviewLayerToView(view, newCameraOutputMode: cameraOutputMode)
    }
    
    @discardableResult  func addPreviewLayerToView(_ view: UIView, newCameraOutputMode: CameraOutputMode) -> CameraState {
        return addLayerPreviewToView(view, newCameraOutputMode: newCameraOutputMode, completion: nil)
    }
    
    @discardableResult  func addLayerPreviewToView(_ view: UIView, newCameraOutputMode: CameraOutputMode, completion: (() -> Void)?) -> CameraState {
        if _canLoadCamera() {
    ...
    func capturePictureWithCompletion(_ imageCompletion: @escaping (UIImage?, NSError?) -> Void) {
    ...
    /**
     ...
     :param: imageCompletion Completion block containing the captured UIImage
     */
    func capturePictureWithCompletion(_ imageCompletion: @escaping (CaptureResult) -> Void) {
     ...
     */
    typealias QRCodeDetectionHandler = (Result<String, Error>) -> Void
    ...
    fileprivate func attachZoom(_ view: UIView) {
        DispatchQueue.main.async {
            self.zoomGesture.addTarget(self, action: #selector(VFCameraManager._zoomStart(_:)))
    ...
    fileprivate func attachFocus(_ view: UIView) {
        DispatchQueue.main.async {
            self.focusGesture.addTarget(self, action: #selector(VFCameraManager._focusStart(_:)))
    ...
    fileprivate func attachExposure(_ view: UIView) {
        DispatchQueue.main.async {
            self.exposureGesture.addTarget(self, action: #selector(VFCameraManager._exposureStart(_:)))
    ...
    func _doFlipAnimation() {
        ...
        if let validEmbeddingView = embeddingView,
           let validPreviewLayer = previewLayer {
            ...
            if VFCameraManager._blurSupported() {
...
extension VFCameraManager: AVCaptureMetadataOutputObjectsDelegate {
    /**
